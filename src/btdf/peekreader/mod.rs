...
use std::io::{BufRead, Error, ErrorKind, Read, Result, Take, Seek, SeekFrom};
...
pub trait PeekRead: BufRead {
    ...
    /// Exactly like [`BufRead::fill_peek`], but fills and returns a buffer at the peek cursor
    /// instead.
    ...
    ///      was moved back and [`peek_position`] is calculated relative to it).
...
/// Iterator type for [`PeekRead::peak_bytes`].
#[derive(Debug)]
